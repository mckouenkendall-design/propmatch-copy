...
export default function GroupMembers({ groupId, currentUserRole }) {
  ...
  // Fetch all UserProfile records so we can look up current names by email
  const { data: userProfiles = [] } = useQuery({
    queryKey: ['all-user-profiles'],
    queryFn: () => base44.entities.UserProfile.list(),
  });

  // Build a map of email -> profile for fast lookup
  const profileMap = Object.fromEntries(
    userProfiles.map(p => [p.user_email, p])
  );

  // Get the best display name for a member: UserProfile first, then stored user_name, then email
  ...
  if (isLoading) return (
    <div style={{ textAlign: 'center', padding: '32px', color: 'rgba(255,255,255,0.4)', fontFamily: "'Inter', sans-serif" }}>
      Loading members...
    </div>
  );
              ...
              return (
                <div key={member.id} style={{ background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: '12px', padding: '16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: '#f59e0b', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '14px', fontWeight: 700 }}>
                  ...
                  <div style={{ display: 'flex', gap: '8px' }}>
                    <button
                      ...
                      style={{ display: 'flex', alignItems: 'center', gap: '4px', padding: '8px 14px', borderRadius: '8px', fontSize: '13px', fontWeight: 500, color: '#111827', background: ACCENT, border: 'none', cursor: 'pointer' }}
                    >
                      <UserCheck style={{ width: '14px', height: '14px' }} /> Approve
                    </button>
                    <button
                      onClick={() => removeMutation.mutate(member.id)}
                      style={{ display: 'flex', alignItems: 'center', gap: '4px', padding: '8px 14px', borderRadius: '8px', fontSize: '13px', fontWeight: 500, color: 'rgba(255,255,255,0.6)', background: 'rgba(255,255,255,0.06)', border: 'none', cursor: 'pointer' }}
                    >
          ...
          {activeMembers.map(member => {
            ...
            return (
              <div key={member.id} style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: '16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: ACCENT, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#111827', fontSize: '14px', fontWeight: 700, flexShrink: 0 }}>
                    {displayName[0]?.toUpperCase() || '?'}
                  </div>
                  <div>
                    <p style={{ fontFamily: "'Inter', sans-serif", fontSize: '14px', fontWeight: 600, color: 'white', margin: 0 }}>{displayName}</p>
