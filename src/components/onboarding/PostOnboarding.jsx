...
function HowDidYouHearScreen({ onNext }) {
  ...
  const handleNext = async () => {
    ...
    try {
      if (user._profileId) {
        await supabase.from('profiles').update({ referral_source: selected }).eq('id', user._profileId);
      }
    } catch {
      // ignore
    }
...
function ThemeScreen({ onNext }) {
  ...
  const handleNext = async () => {
    try {
      if (user._profileId) {
        await supabase.from('profiles').update({ theme_preference: selectedTheme }).eq('id', user._profileId);
      }
    } catch {
      // ignore
    }
