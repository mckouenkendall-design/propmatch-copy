        ...
        <div className="grid grid-cols-2 gap-4" style={{ marginTop: '4px' }}>
          ...
          <Field label="Occupancy Timing">
            <select className={selCls} style={selStyle} value={details.occupancy_type || ''} onChange={e => setDetail('occupancy_type', e.target.value)}>
              <option value="" style={{ background: '#0E1318' }}>Select…</option>
              {OCCUPANCY_TIMING.map(o => <option key={o.value} value={o.value} style={{ background: '#0E1318' }}>{o.label}</option>)}
            </select>
          </Field>
      ...
      <div style={{ border: '1px solid rgba(255,255,255,0.08)', borderRadius: '10px', overflow: 'hidden', marginTop: '4px' }}>
        <button type="button" onClick={() => setCondOpen(o => !o)}
          ...
          <div style={{ textAlign: 'left' }}>
            <span style={{ fontFamily: "'Inter',sans-serif", fontSize: '13px', fontWeight: 700, color: 'rgba(255,255,255,0.85)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Sale Conditions</span>
            {conditions.length > 0
              ? <span style={{ fontFamily: "'Inter',sans-serif", fontSize: '11px', color: ACCENT, marginLeft: '10px' }}>{conditions.length} selected</span>
              : <span style={{ fontFamily: "'Inter',sans-serif", fontSize: '11px', color: 'rgba(255,255,255,0.35)', marginLeft: '10px' }}>optional</span>}
          </div>
