...
export const CLIENT_WEIGHT_DEFAULTS = {
  office: {
    ...
    items: [
      ...
      { key: 'workstations', label: 'Workstations', default: 'normal',
        why: "Workstation count reflects how many desks the open area can hold, which drives how large a team the space actually supports. Adding workstations means furniture and sometimes electrical and data drops. We weight this because it's a direct measure of whether the client's team fits." },
      { key: 'fits_people', label: 'Fits People', default: 'high',
        why: "This is the most direct answer to the question every office tenant is really asking: will my team fit? It rolls offices, workstations, and open area into a single headcount capacity. We weight this high because a space that can't seat the team is off the table regardless of anything else." },
      { key: 'built_out_as', label: 'Built Out As', default: 'normal',
        why: "A space already built out for a similar use — a law firm, a call center, a medical practice — can save six figures and months of construction. A mismatch means gutting and rebuilding. We weight this because inheriting the right build-out is one of the biggest hidden savings in an office deal." },
  ...
  retail: {
    ...
    items: [
      ...
      { key: 'space_position', label: 'Space Position', default: 'normal',
        why: "Where the space sits — strip mall, standalone, in-line, end cap, corner, outparcel, or food court — shapes visibility, signage, co-tenancy, parking access, and the type of customer who arrives. An end cap or outparcel commands a premium because of exposure and drive-thru potential. These are fixed characteristics of the property. We weight this because the right position depends entirely on the business model." },
      { key: 'built_out_as', label: 'Built Out As', default: 'normal',
        why: "A space already built out for a similar use — a restaurant with a hood and grease trap, a bank with a vault, a medical suite — can save six figures and months of construction. A former quick-service space is enormously valuable to the next food tenant. A mismatch means gutting and rebuilding. We weight this because inheriting the right build-out is one of the biggest hidden savings in a retail deal." },
  ...
  industrial_flex: {
    ...
    items: [
      ...
      { key: 'truck_wells', label: 'Truck Wells', default: 'normal',
        why: "Truck wells are the depressed loading positions that let a trailer back down to dock height. The count caps how many trucks can load or unload at once, which directly limits throughput for a distribution operation. Adding wells means excavation and structural work. We weight this because it sets the ceiling on loading capacity." },
      { key: 'dock_levelers', label: 'Dock Levelers', default: 'normal',
        why: "Dock levelers bridge the gap and height difference between the dock and the trailer bed so forklifts can drive on and off safely. The count determines how many docks are actually usable for efficient loading. They can be added, but at real cost per door. We weight this as a practical measure of functional loading capacity." },
      { key: 'floor_thickness', label: 'Floor Thickness', default: 'normal',
        why: "Slab thickness, measured in inches, works together with floor load to determine what heavy equipment and racking the floor supports. A thin slab limits forklift traffic and heavy storage. Thickening a slab means tearing it out and repouring. We weight this because, like floor load, it's a fixed structural constraint on how the space can be used." },
  ...
  manufactured: {
    ...
    items: [
      ...
      { key: 'year_built', label: 'Year Built', default: 'normal',
        why: "A manufactured home's age drives its condition, energy efficiency, and financing options — many lenders won't finance homes built before the 1976 HUD code, and some communities set their own age limits on incoming units. We weight this because age affects both livability and whether the deal can even be financed." },
      { key: 'age_restriction', label: 'Age Restriction', default: 'dealbreaker',
        why: "Age-restricted communities (typically 55+) require that at least one resident meets the age requirement, enforced by the community and not waivable by the seller or landlord. We treat this as a dealbreaker by default because a buyer who doesn't qualify simply cannot live there." },
      { key: 'land_ownership', label: 'Land Ownership', default: 'high',
        why: "Whether the land is owned or the lot is leased changes everything about the deal — an owned lot builds equity and avoids lot rent, while a leased lot means a recurring payment and community rules. We weight this heavily because it fundamentally changes the ownership and cost structure of the purchase." },
      { key: 'lot_rent', label: 'Lot Rent', default: 'high',
        why: "On a leased lot, monthly lot rent is a permanent recurring cost on top of the home price, and it can rise over time at the community's discretion. We weight this heavily because for a leased-lot buyer it's often the single biggest factor in whether the home is affordable long-term." },
      { key: 'utilities_setup', label: 'Utilities Setup', default: 'normal',
        why: "Municipal water and sewer versus well and septic changes both upfront reliability and ongoing maintenance responsibility. Well and septic put testing and repair on the owner. We weight this because the setup affects monthly cost, maintenance burden, and sometimes financing." },
      { key: 'hud_tag', label: 'HUD Tag', default: 'normal',
        why: "The HUD tag and title confirm the home was built to federal manufactured-housing standards, which most lenders require to finance the purchase. A missing tag can make a home difficult or impossible to finance. We weight this because it directly affects financeability." },
      { key: 'permanent_foundation', label: 'Permanent Foundation', default: 'normal',
        why: "A permanent foundation lets a manufactured home be titled as real property rather than a vehicle, which unlocks conventional mortgage financing and typically higher resale value. We weight this because it changes both how the home is financed and how it appreciates." },
      { key: 'central_a_c', label: 'Central A/C', default: 'low',
        why: "Central air is a comfort and resale feature that's expensive to add after the fact in a manufactured home. Its presence is a bonus rather than a hard requirement for most buyers. We score this as additive because window units are a workable alternative." },
      { key: 'carport_garage', label: 'Carport / Garage', default: 'low',
        why: "Covered parking protects vehicles and adds usable outdoor storage, but it can often be added later where community rules allow. We score this as a preference because its absence is usually fixable rather than a dealbreaker." },
      { key: 'covered_porch_deck', label: 'Covered Porch / Deck', default: 'low',
        why: "A covered porch or deck adds livable outdoor space and is a common desire in manufactured-home communities, but it can typically be built after purchase subject to community approval. We score this as additive because it's usually an addition rather than a fixed constraint." },
    ],
  },
};
