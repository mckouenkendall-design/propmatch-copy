...
import { Users, Megaphone, Video, FolderOpen, Building2, Plus, Globe, Lock, Calendar, ExternalLink, Pin, FileText, Download, Mail, Phone, MessageCircle } from 'lucide-react';
...
function MatchBadge({ matchInfo }) {
  if (!matchInfo) return null;
  const sc = getScoreColor(matchInfo.totalScore);
  const sl = getScoreLabel(matchInfo.totalScore);
  return (
    <div style={{ display:'flex', alignItems:'center', gap:'5px' }}>
      <div style={{ width:'8px', height:'8px', borderRadius:'50%', background:sc, flexShrink:0 }}/>
      <span style={{ fontFamily:"'Inter',sans-serif", fontSize:'11px', fontWeight:700, color:sc }}>{matchInfo.totalScore}%</span>
      {sl && <span style={{ fontFamily:"'Inter',sans-serif", fontSize:'10px', fontWeight:600, color:sc, background:`${sc}15`, border:`1px solid ${sc}30`, borderRadius:'4px', padding:'1px 5px' }}>{sl}</span>}
    </div>
  );
}

function PipelineDetailModal({ post, onClose, matchInfo, onMessage }) {
  if (!post) return null;
      ...
      <div style={{ background:'#0E1318', border:'1px solid rgba(255,255,255,0.1)', borderRadius:'20px', width:'100%', maxWidth:'640px', overflow:'hidden', marginBottom:'20px' }}
        ...
        <div style={{ padding:'18px 24px', borderBottom:'1px solid rgba(255,255,255,0.07)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
            ...
            <span style={{ fontFamily:"'Inter', sans-serif", fontSize:'11px', fontWeight:600, textTransform:'uppercase', letterSpacing:'0.06em', color:'rgba(255,255,255,0.4)' }}>
              {isListing ? 'Listing' : 'Requirement'}
            </span>
            {matchInfo && <MatchBadge matchInfo={matchInfo} />}
          </div>
        ...
        <div style={{ padding:'24px' }}>
          ...
          <div style={{ display:'flex', flexWrap:'wrap', gap:'8px', marginBottom:'20px' }}>
            {[post.city && `${post.city}${post.state?', '+post.state:''}`, ...(post.cities||[])].filter(Boolean).map((v,i) => (
              <span key={i} style={{ padding:'4px 10px', background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:'6px', fontFamily:"'Inter', sans-serif", fontSize:'12px', color:'rgba(255,255,255,0.6)' }}>{v}</span>
            ))}
          ...
          <div style={{ background:`${ACCENT}05`, border:`1px solid ${ACCENT}20`, borderRadius:'12px', padding:'14px' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'6px' }}>
              {post.contact_agent_email && (
                <a href={`mailto:${post.contact_agent_email}`} style={{ display:'flex', alignItems:'center', gap:'8px', fontFamily:"'Inter', sans-serif", fontSize:'13px', color:ACCENT, textDecoration:'none', padding:'7px 12px', background:`${ACCENT}08`, borderRadius:'7px', border:`1px solid ${ACCENT}15` }}>
                  <Mail style={{width:'13px',height:'13px',flexShrink:0}} />{post.contact_agent_email}
                </a>
              )}
              {post.contact_agent_phone && (
                <a href={`tel:${post.contact_agent_phone}`} style={{ display:'flex', alignItems:'center', gap:'8px', fontFamily:"'Inter', sans-serif", fontSize:'13px', color:ACCENT, textDecoration:'none', padding:'7px 12px', background:`${ACCENT}08`, borderRadius:'7px', border:`1px solid ${ACCENT}15` }}>
                  <Phone style={{width:'13px',height:'13px',flexShrink:0}} />{post.contact_agent_phone}
                </a>
              )}
              {onMessage && post.contact_agent_email && (
                <button onClick={() => onMessage(null, post.contact_agent_email)}
                  style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:'7px', padding:'8px', background:ACCENT, border:'none', borderRadius:'7px', fontFamily:"'Inter', sans-serif", fontSize:'13px', fontWeight:600, color:'#111827', cursor:'pointer' }}>
...
export default function Teams() {
  ...
  const myListings     = allListings.filter(l => l.created_by === user?.email);
  const myRequirements = allRequirements.filter(r => r.created_by === user?.email);
  ...
  return (
    <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '48px 24px' }}>
                  ...
                  {teamListings.length === 0 ? (
                    ...
                    return (
                      <Card key={listing.id}
                        ...
                        style={{ background: 'rgba(255,255,255,0.04)', border: `1px solid ${matchInfo ? `${scoreColor}35` : 'rgba(255,255,255,0.1)'}`, cursor: 'pointer', transition: 'all 0.2s', position: 'relative' }}
                        onMouseEnter={e => { e.currentTarget.style.borderColor = ACCENT; e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = `0 0 16px ${ACCENT}15`; }}
                        onMouseLeave={e => { e.currentTarget.style.borderColor = matchInfo ? `${scoreColor}35` : 'rgba(255,255,255,0.1)'; e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = 'none'; }}>
                        ...
                        <CardHeader style={{ paddingRight: '120px' }}>
                          {matchInfo && <div style={{ marginBottom: '8px' }}><MatchBadge matchInfo={matchInfo} /></div>}
      ...
      {selectedPost && (() => {
        const matchInfo = brokerageMatchMap[selectedPost.id];
        const isMyPost  = selectedPost.created_by === user?.email;
        return (
          <PipelineDetailModal
            post={selectedPost}
            onClose={() => setSelectedPost(null)}
            matchInfo={!isMyPost ? matchInfo : null}
            onMessage={!isMyPost && selectedPost.contact_agent_email ? (recipientProfile, recipientEmail) => {
              setSelectedPost(null);
              setCompose({
                recipientProfile,
                recipientEmail: recipientEmail || selectedPost.contact_agent_email,
