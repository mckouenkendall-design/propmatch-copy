...
function fmtPrice(p,isL){const n=v=>{const x=parseFloat(v);return(!v||isNaN(x))?null:x%1===0?x.toLocaleString():x.toLocaleString('en-US',{maximumFractionDigits:2});};if(isL){const f=n(p.price);if(!f)return null;const u=p.transaction_type==='lease'||p.transaction_type==='sublease'?'/SF/yr':p.transaction_type==='rent'?'/mo':'';return`$${f}${u}`;}const u=p.price_period==='per_month'?'/mo':p.price_period==='per_year'?'/yr':p.price_period==='per_sf_per_year'?'/SF/yr':p.price_period==='annually'?'/yr':(p.transaction_type==='lease'||p.transaction_type==='rent')?'/mo':'';const lo=n(p.min_price),hi=n(p.max_price);if(lo&&hi)return`$${lo}\u2013$${hi}${u}`;if(hi)return`Up to $${hi}${u}`;if(lo)return`From $${lo}${u}`;return null;}
